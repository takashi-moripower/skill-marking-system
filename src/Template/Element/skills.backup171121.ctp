...
$drawFunc = function($skills, $cardClass = null) use($loginUserId) {
    foreach ($skills as $skill) {
        if ($loginUserId == $skill->marker_id) {
            $cardClass = 'border-dark bg-skill-loginuser';
        }
        $marker_id = Hash::get($skill, '_joinData.user_id', Hash::get($skill, 'marker_id'));

        echo '<div class="card d-inline-block m-1 ' . $cardClass . '" marker_id="' . $marker_id . '"><div class="card-body px-1 py-0">';
...
$drawFunc($skillsSelf, 'border-dark bg-skill-self');

debug( Hash::extract( $skillsSelf , '{n}.id'));
debug( Hash::extract( $skillsOtherMax , '{n}.id'));
